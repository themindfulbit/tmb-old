...
if (is_array($environments)) {
  $environment = Statamic::detect_environment($environments, $app->request()->getUrl());
  if ($environment) {
...
$app->map('/TRIGGER/:add_on/:hook', function($add_on, $hook) use ($app) {
  ...
  $locations = array('_app/statamic/core/', '_add-ons/');
  ...
  foreach ($locations as $folder) {
    ...
    if (is_dir($folder) && is_file($file)) {
      ...
      require_once '_app/statamic/hooks.php';
      require_once($file);
...
$app->map('/(:segments+)', function ($segments = array()) use ($app) {
  ...
  } else if (Statamic::is_taxonomy_url($path)) {
    list($type, $slug) = Statamic::get_taxonomy_criteria($path);
  ...
  if ($add_prev_next && $visible) {
    $prev = Statamic::find_prev($page, $folder);
    $next = Statamic::find_next($page, $folder);

    if ($prev) {
      $prev = Statamic_Helper::remove_numerics_from_path($folder."/".$prev);
      $prev = Statamic_Helper::reduce_double_slashes($prev);
    }

    if ($next) {
      $next = Statamic_Helper::remove_numerics_from_path($folder."/".$next);
      $next = Statamic_Helper::reduce_double_slashes($next);
    }
