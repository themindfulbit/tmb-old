...
class Statamic_User {
  ...
  public function set_password($password, $encrypted=false) {
    if ($encrypted) {
      ...
      if (!isset($this->data['salt']) || $this->data['salt'] == '') {
        $this->data['salt'] = Statamic_Helper::random_string(32);
      }
  ...
  public function set_biography_raw($biography) {
    ...
    $this->data['biography'] = Statamic::transform_content($biography);
  }
  ...
  public static function load($username) {
    ...
    if (stripos($meta_raw, "---") === FALSE) {
      ...
      $meta['biography'] = trim(Statamic::transform_content($content));
