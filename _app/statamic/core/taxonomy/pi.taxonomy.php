...
class Plugin_taxonomy extends Plugin {
  ...
  public function listing() {
    ...
    if ($folder && $type) {
      $list = Statamic::get_content_list($folder, null, null, false, true, $sort_by, null, '', null, false, false, null, null);

      $results = array();
      $urls   = array();
      $initialize = $app->config;
      foreach ($list as $key => $item) {
        $content = Statamic::transform_content($item['content']);
      ...
      if ($sort_by == 'count') {
        ...
        ksort($results, SORT_STRING | 'SORT_FLAG_CASE');
      }
