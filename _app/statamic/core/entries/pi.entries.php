...
class Plugin_entries extends Plugin {
  ...
  public function listing() {
    ...
    $taxonomy_slugify = Statamic::get_setting('_taxonomy_slugify', false);
    ...
    if ($taxonomy) {
      list($type, $tax_slug) = Statamic::get_taxonomy_criteria($app->request()->getResourceUri());
      $tax_slug = $taxonomy_slugify ? Statamic_Helper::deslugify($tax_slug) : urldecode($tax_slug);
    ...
    if ($folder) {
      $list = Statamic::get_content_list($folder, $limit, $offset, $show_future, $show_past, $sort_by, $sort_dir, $conditions, $switch, false, false, $since, $until);
      if (sizeof($list)) {
  ...
  public function pagination() {
    ...
    if ($taxonomy) {
      ...
      if($taxonomy_slugify) {
        $tax_slug = Statamic_Helper::deslugify($tax_slug);
      } else {
    ...
    $count = Statamic::get_content_count($folder, $show_future, $show_past, $conditions, $since, $until);
